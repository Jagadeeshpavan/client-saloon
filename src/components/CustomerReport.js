...
const CustomerReport = () => {
  ...
  const [, setAgeChartVisible] = useState(false);
  const [selectedAge, setSelectedAge] = useState(null);
  const [, setAgeChartData] = useState(null);
  ...
  const getCustomersWithAge = useCallback((age) => {
    return customers.filter((customer) => calculateAge(customer.dob) === age);
  }, [customers, calculateAge]);
  ...
  const calculateServicesCountAndRevenue = useCallback((age) => {
    const customersWithAge = getCustomersWithAge(age);
    let serviceCount = 0;
    let totalRevenue = 0;
  ...
  }, [getCustomersWithAge, customerData]);
  ...
  }, [customers, calculateServicesCountAndRevenue,calculateAge]);
