...
const CustomerDetails = ({ selectedCustomer }) => {
  ...
  const handleDelete = async (appointmentId) => {
    ...
    try {
      ...
      await Axios.delete(`${BASE_URL}/api/customers/${customerId}/appointments/${appointmentId}`);
  ...
  const filteredBills = customer.billing.filter((bill) =>
   bill.billNumber && bill.billNumber.includes(searchBillQuery)
  );

  const handleEditSave = async (editedCustomer) => {
    setCustomer(editedCustomer);
    try {
      const response = await fetch(`${BASE_URL}/api/customers/${editedCustomer._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleEditBillSave = async (customerData) => {
    try {
      const response = await fetch(`${BASE_URL}/api/customers/billing/${customerData._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },  
            ...
            <div className="mdiv">
              <div className="sdiv1">
                ...
                {customer.profilePhoto ? (
                  <img
                    className="sdiv1img"
                    src={`${BASE_URL}/${customer.profilePhoto}`}
                    alt="Customer Avatar"
                  />
                ) : (
