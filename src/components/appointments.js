...
const filteredCustomers = customers.filter((customer) => {
  ...
  const updatedCustomers = customers.map((customer) => {
    const filteredAppointments = customer.appointments.filter((appointment) =>
      appointment.name.toLowerCase().includes(searchAppointmentQuery.toLowerCase()) ||
  ...
  const filteredAppointments = updatedCustomers.flatMap((customer) => customer.appointments);
  ...
  const updatedCustomers = customers.map((customer) => {
    const todayAppointments = customer.appointments.filter((appointment) => appointment.date === today);
    ...
    const fetchCustomers = async () => {
      try {
        ...
        const todayAppointments = response.data.map((customer) => ({
          ...customer,
          appointments: customer.appointments.filter((appointment) => appointment.date === today),
        }));
...
const handleClick = (customerData) => {
  ...
  const handleEdit = (customer,item) => {
    ...
    setDisplayComponent('editAppointment');
  };
  ...
  const handleEditSave = async (customerData) => {
    try {
      const response = await fetch(`${BASE_URL}/api/appointments/${customerData._id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        const customerIndex = updatedCustomers.findIndex((customer) => customer._id === customerData._id);
  
        if (customerIndex !== -1) {
          const appointmentIndex = updatedCustomers[customerIndex].appointments.findIndex(
            (appointment) => appointment._id === customerData.appointments[0]._id
          );
  
          if (appointmentIndex !== -1) {
            updatedCustomers[customerIndex].appointments[appointmentIndex] = customerData.appointments[0];
            setCustomers(updatedCustomers);
          }
        }
    ...
    try {
      const confirmToastId = toast(
        <div>
          ...
          <button  className="confirm-btn confirm-yes"
            onClick={() => {
          ...
          <button  className="confirm-btn confirm-no"
            onClick={() => {
  ...
  const handleConfirmDelete = async (confirmToastId, customerId, appointmentId) => {
    try {
      toast.dismiss(confirmToastId);
  
      await axios.delete(
       ` http://localhost:5000/api/customers/${customerId}/appointments/${appointmentId}`
      );
  ...
  const currentAppItems = updatedCustomers.slice(indexOfAppFirstItem, indexOfAppLastItem);
 ...
 <tbody>
              {currentItems.map((customer) => (
                <tr key={customer.email}>
                   <td className='customer-table11-td1'>{customer.customerId}</td>
                  <td className='customer-table11-td'>{customer.name}</td>
                  <td className='customer-table11-td1'>{customer.phone}</td>
                  <td className='customer-table11-td'>{customer.address}</td>
                  ...
                  <button className='book-text' onClick={() => handleClick(customer)}>Book</button>
...
<div className='customer-container11'>
  ...
  <div className='select-number-of-entries'>
  <label>Show </label>
            <select className="input1" value={appsPerPage} onChange={handleAppsPerPageChange} >
              <option value={5}>5</option>
              <option value={10}>10</option>
              <option value={15}>15</option>
              {/* Add more options as needed */}
...
<div className='from-div'>From &nbsp;<input
  type='date'
  id='fromDate'
...
<button className='fst filter' onClick={handleFilter}>Filter</button>

</div>
<div className='app-filter-buttons11'>
  
  <button className='fst filter'onClick={handleShowAll}>Show all</button>
  <button className='fst today' onClick={handleShowToday}>Today</button>
        ...
        {currentAppItems.map((customer,customerIndex) => (
          customer.appointments.map((item, index) => (
            <tr key={item.id}>
              ...
              <td className='customer-table11-td1'>{formatDate(item.date)}</td>
              <td className='customer-table11-td1'>{item.fromTiming}</td>
              <td className='customer-table11-td1'>{item.toTiming}</td>
      ...
      <ol>
        {item.selectedServices.map((service, serviceIndex) => (
          <li key={serviceIndex}>{service}</li>
             ...
             <td className='customer-table11-td1'>
             <button className='app-edit-btn11' onClick={() => handleEdit(customer,item)}>Edit</button>
             <button className='app-delete-btn11' onClick={() => handleDelete(customer._id, item._id)}>Delete</button>
        ...
        <div className="entries-div121">
          <div className='number-of-entries-div'>
          Showing {indexOfAppFirstItem + 1} to {Math.min(indexOfAppLastItem, filteredAppointments.length)} of {filteredAppointments.length} entries
         
          </div>
        <div>
          <button className="badges" onClick={handleAppFirstPageClick}>First</button>
          <button className="badges" onClick={handleAppPreviousPageClick}>Previous</button>
          {getDisplayedAppPages().map((appPageNumber) => (
            <button
              key={appPageNumber}
              className={`badges ${appPageNumber === currentAppPage ? 'active' : ''}`}
              onClick={() => handleAppPageChange(appPageNumber)}
            >
              {appPageNumber}
            </button>
          ))}
          <button className="badges" onClick={handleAppNextPageClick}>Next</button>
          <button className="badges" onClick={handleAppLastPageClick}>Last</button>
        </div>
        </div>
...
{editIndex !== null && (
  <EditAppointments
    data={editIndex.customer} 
     ...
     index={editIndex.customer.appointments.findIndex((appointment)=>appointment._id === editIndex.item._id)} 
