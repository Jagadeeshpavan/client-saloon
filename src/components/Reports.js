...
function BillingTable() {
  ...
  const updateChartData = (viewMode, data) => {
    const aggregateData = (mode) => {
      ...
      setChartData({
        ...
        datasets: [
          {
            label: `${viewMode.charAt(0).toUpperCase() + viewMode.slice(1)}ly Income`,
            backgroundColor: 'rgba(75, 192, 192, 0.2)',
            borderColor: 'rgba(75, 192, 192, 1)',
            borderWidth: 1,
            data: values,
  ...
  const currentItems = filteredBillingData.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const handleExport = () => {
    ...
    const ws1 = XLSX.utils.table_to_sheet(document.querySelector('.billing-table-sk654s'));
...
<div className='button456'>
          ...
          <div className='btns-783'>
            <button onClick={handleClickShowAll} className='show-all-button-sk654s'>
              Show All
            </button>&nbsp;
            <button onClick={handleShowToday} className='show-today-button-sk654s'>
              Today
            </button>&nbsp;
            <button onClick={toggleChartVisibility} className='show-chart-button'>
        ...
        <div className='pie-chart-container'>
          {showChart && (
            <div className='bar-graph-container-cvvroyalrpt'>
              <canvas ref={chartContainer} id='income-chart' className='chartrange574'></canvas>
            </div>
          )}
        </div>
        </div>
            ...
            <tbody>
              {currentItems.map((billing) => (
                <tr className='customer-table11-td1' key={billing._id}>
                  ...
                  <td className='billing-table-cell-sk654s'>{billing.customer}</td>
                  <td className='customer-table11-td1'>{formatDate(billing.date)}</td>
                  <td className='customer-table11-td1'>{billing.discountPercent}%</td>
