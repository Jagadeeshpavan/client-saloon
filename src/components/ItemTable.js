...
function ItemTable() {
  ...
  }, [showToday, showPieChart, calculatePieChartData]);
  ...
  //     handleShowToday();
  //   }
  // }, [showAll, showToday, handleShowAll, handleShowToday, fetchData]);
  ...
  const handleShowAll = useCallback(() => {
    ...
    const pieChartInfo = calculatePieChartData(billingData);
    setPieChartData(pieChartInfo);
  }, [billingData, setFromDate, setToDate, setShowAll, setShowToday, calculatePieChartData]);
  ...
  }, [showAll, showToday, handleShowAll, handleShowToday]);
  ...
  const totalBillAmount = filteredBillingData.reduce((total, bill) => {
    return total + bill.items.reduce((subtotal, item) => subtotal + (item.price * item.quantity), 0);
  }, 0);
  ...
  const handleExport = () => {
    ...
    const ws1 = XLSX.utils.table_to_sheet(document.getElementById('item-table'));
    ...
    <div className="billing-table-container-sk654s">
      ...
      <div className="flex-109586">
        <div className="flex-1095">
          ...
          <label htmlFor="fromDate" className="date-label-sk654s">From Date:</label>
          <input
            type="date"
            id="fromDate"
          ...
          <label htmlFor="toDate" className="date-label-sk65432s">To Date:</label>
          <input
            type="date"
            id="toDate"
            className="date-input-sk654s"
        ...
        <div className='flex-change6743'>
            <button onClick={handleShowAll} className='show-all-button-sk654s'>Show All</button>&nbsp;
          <button onClick={handleShowToday} className="show-today-button-sk654s">
            Today
          </button>&nbsp;
        ...
        <tbody>
          {currentPageData.map((bill, index) => (
            <tr key={index} className="billing-table-row-sk654s">
              <td className="billing-table-cell-sk654s">{index + 1 + (currentPage - 1) * rowsPerPage}</td>
              <td className="customer-table11-td">{bill.items.map(item => item.itemName).join(', ')}</td>
              <td className="billing-table-cell-sk654s">{bill.items.map(item => item.price).join(', ')}</td>
              <td className="billing-table-cell-sk654s">{bill.items.map(item => item.quantity).join(', ')}</td>
              <td className="billing-table-cell-sk654s">{formatTotalPrice(bill.items.reduce((total, item) => total + (item.price * item.quantity), 0))}</td>
              <td className="billing-table-cell-sk654s">{formatDate(bill.date)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <p className='para7890'>Total Amount: Rs {formatTotalPrice(totalBillAmount)}</p>
    </div>
  );
}
