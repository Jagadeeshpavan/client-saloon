...
const ServiceForm = ({ onNewServiceClick }) => {
  ...
  const handleDelete = async (selectedServiceId) => {
    ...
    toast.info(
      <>
        ...
        <button onClick={() => handleConfirmDelete(selectedServiceId)}>Yes</button>
  ...
  const handleConfirmDelete = async (selectedServiceId) => {
    ...
    try {
      ...
      const updatedServices = services.filter((service) => service._id !== selectedServiceId);
      setServices(updatedServices);
      setDisplayComponent('Services');
      setSelectedServiceId('');
...
<button type="button" onClick={()=>handleDelete(selectedServiceId)} className="delete-button-sk142sk">
        Delete
      </button>
        ...
        <div className='changeedit248'>
          {isEditing!==null && (
            <EditService
              selectedService={services.find((service) => service._id === selectedServiceId)}
