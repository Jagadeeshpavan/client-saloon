...
function EmployeeTable() {
  ...
  const calculateTotalAmount = useCallback((data) => {
  ...
  const generateSelectedEmployeeData = () => {
    if (selectedEmployee && employeeData[selectedEmployee]) {
      ...
      const labels = selectedEmployeeData.services.map((service) => service.serviceName);
      const values = selectedEmployeeData.services.map((service) => service.price || 0);
  ...
  const handleShowToday = useCallback(() => {
    ...
    calculateTotalAmount(updatedEmployeeData);
    setCurrentPage(1);
  }, [setShowAll, setShowToday, setFromDate, setToDate, customerData, calculateTotalAmount, setCurrentPage]);
  

  const handleExport = () => {
    const wb = XLSX.utils.book_new();
    const ws1 = XLSX.utils.table_to_sheet(document.getElementById('employee-table'));
...
<div className='button456'>
          ...
          <div className='flex-change6743'>
            ...
            <button onClick={() => setShowChart(!showChart)} className="show-chart-button"
>
  ...
  <div className="selected-employee-chart">
    ...
    />
    <div className='pie-chart-container'>
    <button onClick={() => setSelectedEmployee('')} className="delete-button-sk142sk">
      Close
    </button>
  </div>
  </div>
              ...
              if (data.services.length > 0) {
                return (
                  <tr key={name}>
                    ...
                    <td className="billing-table-cell-sk6543s colorblue00" 
                    onClick={() => handleEmployeeClick(name)}>
                      {name}
                    </td>
                    <td className="billing-table-cell-sk6543s">
                      {data.services.map((service) => service.serviceName).join(', ')}
                    </td>
                    <td className="billing-table-cell-sk654s">{data.services.length}</td>
                    <td className="billing-table-cell-sk654s">{data.totalAmount.toFixed(2)}</td>
                  </tr>
                );
              }
              return null;
